...
class Meta(nn.Module):
    ...
    def forward(self, x_spt, y_spt, x_qry, y_qry):
        ...
        for i in range(task_num):
            ...
            with torch.no_grad():
                # [setsz, nway]
                logits_q = self.net(x_qry[i], vars=self.net.parameters(), bn_training=True)
            ...
            with torch.no_grad():
                # [setsz, nway]
                logits_q = self.net(x_qry[i], vars=fast_weights, bn_training=True)
                loss_q = F.cross_entropy(logits_q, y_qry[i])
                losses_q[1] += loss_q
                # [setsz]
                pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
    ...
    def finetunning(self, x_spt, y_spt, x_qry, y_qry):
        ...
        with torch.no_grad():
            # [setsz, nway]
            logits_q = net(x_qry, vars=net.parameters(), bn_training=True)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
            # scalar
        ...
        with torch.no_grad():
            # [setsz, nway]
            logits_q = net(x_qry, vars=fast_weights, bn_training=True)
            # [setsz]
            pred_q = F.softmax(logits_q, dim=1).argmax(dim=1)
            # scalar
            correct = torch.eq(pred_q, y_qry).sum().item()
